...
public class ClientesController : ControllerBase
{
    ...
    [HttpPatch("{id:long}")]
    public async Task<ActionResult<ClienteResponseDto>> ActualizarParcial(long id, [FromBody] ActualizarClienteParcialDto dto)
    {
        _logger.LogInformation("Recibida solicitud PATCH para cliente con ID {ClienteId}", id);
        var cliente = await _clienteService.ActualizarClienteParcialAsync(id, dto);
        return Ok(ToResponse(cliente));
    }
